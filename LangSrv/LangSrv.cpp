...
// std::unordered_map< uintptr_t, int > g_Disassemblies; // WIP
		...
		try
		{
			...
			config.m_IdentifierCb = []( int lineNr, int colNr, Compiler::Variable_t& variable, const std::string& context ) -> void {
				...
				if ( variable.m_IsConst && variable.m_Function )
				{
					...
					auto disasm = Disassembler::DisassembleChunk( *chunk, variable.m_Function->GetName() );

					g_SymbolList += ", \"disassembly\": { \"name\": \"" + disasm.m_Name + "\", \"opcodes\": [";
					...
					for ( size_t i = 0; i < disasm.m_Opcodes.size(); ++i )
					{
						...
						g_SymbolList += "{ \"full\": \"" + opcode.m_Full + "\""
